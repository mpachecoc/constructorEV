...
$(document).ready(function(){
    ...
    $('input#doc-name').val('Planilla de Avance de Obra Nro. ' + helper.adv_form_num);
    var id = ACTIVE_PROJECT + '&' + helper.adv_form_num;

    // Get "Advanced Form" data and fill inputs 
    getSingle('advanced_form',id,function(resp) {
        if (resp != 404){
            $.map(resp, function (obj, i) {
                $('input#date-ini').val(formatDate(obj.date_ini));
                $('input#date-end').val(formatDate(obj.date_end));
            });
        }
    });
   ...
   getAllByProj('groups',function(resp) {
                                ...
                                widget = widget + 
                                    '<tr>' +
                                        ...
                                        '<td class="text-right line-left-s3">' +
                                            '<input type="text" id="'+data.id+'" class="form-control adv-form-in" value="'+cant_s3+'" />' +
                                        '</td>' +
                                        '<td class="text-right" id="'+data.id+'-tot-s3">' + formatNum(total_s3) + '</td>' +
                                        '<td class="text-center line-right-s3" id="'+data.id+'-perc-s3">' + formatNum(percent_s3) +
                                            '<div class="progress progress-mini">' +
        ...
        $('#save-adv-form').click(); // Simulate a click to calculate the last widget (TOTALS)
...
$(document).off('click', "#save-adv-form").on('click', "#save-adv-form", function(event) {
    ...
    if(!$('#date-ini').val() || !$('#date-end').val()) {
        ...
        JSONData['id'] = helper.adv_form_num;
        JSONData['date_ini'] = $('#date-ini').val();
        JSONData['date_end'] = $('#date-end').val();
        JSONData = JSON.stringify(JSONData); 

        // Update in DB
        saveInDB('advanced_form','PUT','u',JSONData);
        ...
        $('input.adv-form-in').each(function (i) {
            ...
            if($(this).val()){
                ...
                JSONData['adv_form_id'] = helper.adv_form_num;
                        ...
                        if (resp != 404){        
                            $.map(resp, function (obj, i) {
                                $.each(obj, function (i, arr) { 
                                    task_perc += Number(arr.percent);                                                    
                                });
                            });
                        }
                    });

                    // Patch 'Tasks' Table DB with %
                    var JSONTasks = {};
                    JSONTasks['project_id'] = ACTIVE_PROJECT;
                    JSONTasks['id'] = apu_id;
                    JSONTasks['taskPercentageComp'] = task_perc;
...
// Preview 'Planilla de Avance' to print
$(document).off('click', "#preview-adv-form").on('click', "#preview-adv-form", function(event) {
